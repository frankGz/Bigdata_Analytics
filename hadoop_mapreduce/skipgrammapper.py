...
for line in sys.stdin:
	...
	k = 1
	n = 2
	wdCount = len(filtered)
	index = (n-1)*(k+1) #index difference between first gram to last gram
	while index < wdCount:
		gramRem = n # how many gram (words) per group still remain to read
		gram = ""
		while gramRem > 0:
			gram = gram + ' ' + filtered[index - (gramRem-1)*(k+1)]
			gramRem = gramRem - 1
		index = index + 1
		print(gram.lstrip() + '\t' + '1')
